...
def NMS(tableHit, scoreThreshold=0.5, N_object=float("inf"), maxOverlap=0.5):
    """
...
def _multi_compute(template, image, method, N_object, score_threshold):
    """
    ...
    for peak in peaks :
        ...
        listHit.append({'TemplateName':"template", 'BBox': ( int(peak[1]), int(peak[0]), width, height ) , 'Score':corrMap[tuple(peak)]}) # empty df with correct column header
    return pd.DataFrame(listHit)

def matchTemplates(template, image, method=cv2.TM_CCOEFF_NORMED, N_object=8, score_threshold=0.5, maxOverlap=0.25):
    """
    ...
    tableHit = _multi_compute(template, image, method, N_object, score_threshold)
