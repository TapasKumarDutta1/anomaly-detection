...
class STPM(pl.LightningModule):
    ...
    def cal_loss(
        ...
        for i in range(len(ft_list)):
            ...
            f_loss = 0.5 / (w * h) * torch.sum(wts
                    * torch.sum(self.criterion(fs_norm, ft_norm), [1,
                    2, 3]), 0)
            tot_loss += f_loss
    ...
    def configure_optimizers(self):
        return torch.optim.SGD(self.model_s.parameters(), lr=self.lr,
    ...
    def train_dataloader(self):
        image_datasets = MVTecDataset(root=self.dataset_path,
                transform=self.data_transforms, wts=[1, 0.02, 2])
...
if __name__ == '__main__':
    ...
    trainer = pl.Trainer.from_argparse_args(args,
            ...
            args.category), max_epochs=args.num_epochs, gpus=[0])
