...
class MVTecDataset(Dataset):
    ...
    Args:
        ...
        """
    def __init__(self, root, transform, wts):
    ...
    def __getitem__(self, idx):
        ...
        try:
          #images generated from template has the first digit in their name as number thus check if image was generated
          generated = int(img_path.split("/")[-1].split(".")[0])

          #change the assigned weights
          wt = self.wts[1]
        except:
          #if current image is the template reassign weight
          if img_path.split("/")[-1].split(".")[0]=='template':
            wt = self.wts[2]
...
class STPM(pl.LightningModule):
    ...
    Args:
      ...
      """
    def __init__(self, dataset_path, load_size = 768, lr = 0.4, momentum = 0.9, batch_size = 32, weight_decay = 1e-4, amap_mode='mul'):
    ...
    def cal_loss(self, fs_list, ft_list, wts):
        tot_loss = 0
        for i in range(len(ft_list)):
            ...
            f_loss = (0.5/(w*h))*torch.sum(wts*torch.sum(self.criterion(fs_norm, ft_norm),[1,2,3]),0)
            tot_loss += f_loss
    ...
    def configure_optimizers(self):
        return torch.optim.SGD(self.model_s.parameters(), lr=self.lr, momentum=self.momentum, weight_decay=self.weight_decay)

    #create training dataloader
    def train_dataloader(self):
        image_datasets = MVTecDataset(root=self.dataset_path, transform=self.data_transforms, wts=[1,0.02,2])
        train_loader = DataLoader(image_datasets, batch_size=self.batch_size, shuffle=True, num_workers=0) #, pin_memory=True)
        return train_loader
...
def get_args():
    ...
    parser.add_argument('--project_path', default=r'D:\Project_Train_Results\mvtec_anomaly_detection\210624\test') 
...
if __name__ == '__main__':
    ...
    #create trainer
    trainer = pl.Trainer.from_argparse_args(args, default_root_dir=os.path.join(args.project_path, args.category), max_epochs=args.num_epochs, gpus=[0]) 
    
    #create and fit the model
    model = STPM(dataset_path = args.dataset_path, load_size = args.load_size, lr = args.lr, momentum = args.momentum, batch_size = args.batch_size, weight_decay = args.weight_decay, amap_mode = args.amap_mode)
    trainer.fit(model)
