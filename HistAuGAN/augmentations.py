...
def generate_hist_augs(
    ...
    # compute attribute
    if same_attribute:
        mu, logvar = model.enc_a.forward(
            img.sub(0.5).mul(2).unsqueeze(0),
            torch.eye(5)[img_domain].unsqueeze(0).to(device),
        )
    ...
    out = (
        model.gen(z_content, z_attr, new_domain).detach().squeeze(0)
    )  # in range [-1, 1]
