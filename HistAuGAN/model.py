...
class MD_multi(nn.Module):
    def __init__(self, opts):
        ...
        if self.concat:
            self.enc_a = networks.MD_E_attr_concat(
                opts.input_dim,
                output_nc=self.nz,
                c_dim=opts.num_domains,
                norm_layer=None,
        ...
        self.disContent_opt = torch.optim.Adam(
            self.disContent.parameters(),
            lr=lr / 2.5,
            betas=(0.5, 0.999),
            weight_decay=0.0001,
        )
    ...
    def forward(self):
        ...
        if self.concat:
            ...
            std = torch.exp(self.logvar.mul(0.5))
            eps = self.get_z_random(std.size(0), std.size(1), "gauss").to(device)
            self.z_attr = torch.add(eps.mul(std), self.mu)
        #             print('self.z_attr')
        #             print(self.z_attr)
        else:
        ...
        # get random z_a
        self.z_random = self.get_z_random(half_size, self.nz, "gauss").to(device)
        ...
        if self.concat:
            ...
            eps_recon = self.get_z_random(
                std_recon.size(0), std_recon.size(1), "gauss"
            ).to(device)
            self.z_attr_recon = torch.add(eps_recon.mul(std_recon), self.mu_recon)
        else:
        ...
        self.image_display = torch.cat(
            (
                ...
                self.fake_B_recon[0:1].detach().cpu(),
            ),
            dim=0,
        )
    ...
    def backward_EG(self):
        ...
        loss_G_L1_self = (
            torch.mean(
                torch.abs(
                    self.input
                    - torch.cat((self.fake_AA_encoded, self.fake_BB_encoded), 0)
                )
            )
            * self.opts.lambda_rec
        )
        loss_G_L1_cc = (
            torch.mean(
                torch.abs(
                    self.input - torch.cat((self.fake_A_recon, self.fake_B_recon), 0)
                )
            )
            * self.opts.lambda_rec
        )
    ...
    def backward_G_alone(self):
        ...
        if self.concat:
            ...
            loss_z_L1_a = (
                torch.mean(torch.abs(self.z_attr_random_a - self.z_random)) * 10
            )
            loss_z_L1_b = (
                torch.mean(torch.abs(self.z_attr_random_b - self.z_random)) * 10
            )
