...
class MD_E_attr_concat(nn.Module):
    def __init__(self, input_dim, output_nc=8, c_dim=3, norm_layer=None, nl_layer=None):
        ...
        conv_layers += [
            nn.Conv2d(
                input_dim + c_dim, ndf, kernel_size=4, stride=2, padding=0, bias=True
            )
        ]
...
class MD_G_uni(nn.Module):
    def __init__(self, output_dim, c_dim=3):
        ...
        tch = tch // 2
        dec += [
            nn.ConvTranspose2d(tch, output_dim, kernel_size=1, stride=1, padding=0)
        ] + [nn.Tanh()]
...
class MD_Dis(nn.Module):
    def __init__(self, input_dim, norm="None", sn=False, c_dim=3, image_size=216):
        ...
        self.conv1 = nn.Conv2d(
            curr_dim, 1, kernel_size=1, stride=1, padding=1, bias=False
        )
    ...
    def _make_net(self, ch, input_dim, n_layer, norm, sn):
        ...
        model += [
            LeakyReLUConv2d(
                input_dim, ch, kernel_size=3, stride=2, padding=1, norm=norm, sn=sn
            )
        ]  # 16
        tch = ch
        for i in range(1, n_layer - 1):
            model += [
                LeakyReLUConv2d(
                    tch, tch * 2, kernel_size=3, stride=2, padding=1, norm=norm, sn=sn
                )
            ]  # 8
            tch *= 2
        model += [
            LeakyReLUConv2d(
                tch, tch, kernel_size=3, stride=2, padding=1, norm="None", sn=sn
            )
        ]  # 2
...
class MD_Dis_content(nn.Module):
    def __init__(self, c_dim=3):
        ...
        model += [
            LeakyReLUConv2d(
                256, 256, kernel_size=7, stride=2, padding=1, norm="Instance"
            )
        ]
        model += [
            LeakyReLUConv2d(
...
def conv3x3(in_planes, out_planes):
    return [
        nn.ReflectionPad2d(1),
        nn.Conv2d(in_planes, out_planes, kernel_size=3, stride=1, padding=0, bias=True),
    ]
...
class MisINSResBlock(nn.Module):
    def conv3x3(self, dim_in, dim_out, stride=1):
        return nn.Sequential(
            nn.ReflectionPad2d(1),
            nn.Conv2d(dim_in, dim_out, kernel_size=3, stride=stride),
        )
    ...
    def __init__(self, dim, dim_extra, stride=1, dropout=0.0):
        ...
        self.conv1 = nn.Sequential(
            self.conv3x3(dim, dim, stride), nn.InstanceNorm2d(dim)
        )
